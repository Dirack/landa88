...
void test_interfaceSetupInConstantVelocityModel()
/*< Test model setup in a constant velocity model: NIP sources position >*/
{
	float **s;
	int ns=4;
	float m0[4]={5.,5.,5.,5.};
	...
	for(i=0;i<ns;i++)
		teta[i]=a[i]*SF_PI/180.;

	interfaceSetup(s,ns,m0,t0,teta,0,n,d,o,slow);
...
void test_interfaceSetupInTwoLayersModel()
/*< Test model setup in a constant velocity model: NIP sources position >*/
{
	float **s;
	int ns=4;
	...
	interfaceSetup(s,ns,m0,t0,teta,0,n,d,o,slow2);
...
int main(void){
	...
	init2();
        UNITY_BEGIN();
        RUN_TEST(test_interfaceSetupInConstantVelocityModel);
	RUN_TEST(test_interfaceSetupInTwoLayersModel);
        return UNITY_END();
}
